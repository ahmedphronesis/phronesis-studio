...
# In this sandbox, system env DATABASE_URL is set to a stale SQLite path.
# Force-load DATABASE_URL from .env so Prisma CLI and Next.js both see the Neon URL.
if [ -f "$PROJECT_DIR/.env" ]; then
        export DATABASE_URL="$(grep -E '^DATABASE_URL=' "$PROJECT_DIR/.env" | head -1 | sed -E 's/^DATABASE_URL=//; s/^"(.*)"$/\1/; s/^'"'"'(.*)'"'"'$/\1/')"
fi
bun run db:push || echo "[BUN] db:push skipped (continuing)"
log_step_end "bun run db:push"

log_step_start "Starting Next.js dev server"
echo "[BUN] Starting development server..."
# Re-export DATABASE_URL before launching dev server (Next.js does not override system env).
if [ -f "$PROJECT_DIR/.env" ]; then
        export DATABASE_URL="$(grep -E '^DATABASE_URL=' "$PROJECT_DIR/.env" | head -1 | sed -E 's/^DATABASE_URL=//; s/^"(.*)"$/\1/; s/^'"'"'(.*)'"'"'$/\1/')"
fi
# Use setsid + nohup + disown to fully detach the dev server from this script's process group
# (otherwise it dies when the parent shell exits).
# Use --webpack because Turbopack has a known issue resolving @prisma/client in dev.
setsid nohup bun run dev -- --webpack > "$PROJECT_DIR/dev.log" 2>&1 < /dev/null &
DEV_PID=$!
disown
