          ...
          <div className="relative w-full px-6 md:px-12 lg:px-20 pt-20 md:pt-28 pb-12">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start mb-16">
              ...
              <div className={`lg:col-span-8 ${isAR ? "text-right" : ""}`} dir={isAR ? "rtl" : "ltr"}>
                ...
                <div className={`flex flex-col gap-3 mb-8 p-4 sm:p-5 rounded-2xl bg-paper-warm border border-teal/20 ${isAR ? "items-end" : "items-start"}`} dir={isAR ? "rtl" : "ltr"}>
                  <div className={`text-xs text-ink-dim mb-1 w-full ${isAR ? "text-right" : ""}`}>
                    {t("availableGlobally")}
                  </div>
                  {book.editions.map((ed, i) => (
                    <div key={i} className={`flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 w-full ${isAR ? "sm:flex-row-reverse" : ""}`}>
                      <div className={`flex items-center gap-3 sm:gap-4 ${isAR ? "flex-row-reverse" : ""}`}>
                        <span className="text-xs uppercase tracking-[0.15em] text-ink-dim font-mono min-w-[70px] sm:min-w-[80px] flex-shrink-0">
                      ...
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className={`${isAR ? "sm:mr-auto" : "sm:ml-auto"} inline-flex items-center justify-center gap-2 bg-teal hover:bg-teal-bright text-paper text-sm font-medium px-5 py-2.5 rounded-full transition-colors whitespace-nowrap w-full sm:w-auto sm:flex-shrink-0`}
                      >
